...
export function SmithingMethodsPanel({
  ...
  const ranked = useMemo(() => {
    const list: Ranked[] = SMITHING_METHODS.map((method) => {
      ...
      const baselineOutValue = baselineOutUnit == null ? null : baselineOutUnit * outQty;
      ...
      const locked = smithingLevel != null && smithingLevel < method.level;
    ...
    return list.sort((a, b) => {
      if (smithingLevel != null && a.locked !== b.locked) return a.locked ? 1 : -1;
      let cmp = 0;
      switch (sort) {
        case "gp_desc":
  ...
  }, [rowsByName, trendsById, g, sort, smithingLevel]);
      ...
      <div className="panel flex flex-col gap-3 p-3 sm:p-4">
        <div className="flex flex-wrap items-start justify-between gap-2">
          <div className="min-w-0 flex-1">
            <h2 className="text-sm font-semibold">Smithing methods</h2>
            <p className="text-xs text-muted-foreground">
              Sorted by what each XP costs <em>you</em> — supplies plus the gold you could have made
              instead. Lower cost is better. Blast Furnace coal is already halved.
              {smithingLevel != null && (
                <>
                  {" "}
                  Methods above your Smithing level ({smithingLevel}) are greyed out.
                </>
              )}
            </p>
          </div>
