...
import { CONSTRUCTION_METHODS, type ConstructionMethod } from "@/lib/construction-methods";
...
export function ConstructionMethodsPanel({
  rowsByName,
  trendsById,
  moneyPerHour,
  ...
  const ranked = useMemo(() => {
    const list: Ranked[] = CONSTRUCTION_METHODS.map((method) => {
      ...
      const locked = constructionLevel != null && constructionLevel < method.level;
    ...
    return list.sort((a, b) => {
      if (constructionLevel != null && a.locked !== b.locked) return a.locked ? 1 : -1;
      let cmp = 0;
      switch (sort) {
        case "gp_desc":
  ...
  }, [rowsByName, trendsById, g, sort, constructionLevel]);
      ...
      <div className="panel flex flex-col gap-3 p-3 sm:p-4">
        <div className="flex flex-wrap items-start justify-between gap-2">
          <div className="min-w-0 flex-1">
            <h2 className="text-sm font-semibold">Construction methods</h2>
            <p className="text-xs text-muted-foreground">
              Sorted by what each XP costs <em>you</em> — supplies plus the gold you could have made
              instead. Lower cost is better. POH methods consume planks (no sell-back).
              {constructionLevel != null && (
                <>
                  {" "}
                  Methods above your Construction level ({constructionLevel}) are greyed out.
                </>
              )}
            </p>
          </div>
...
function MethodRow({
  ...
  return (
    <article
      ...
      title={locked ? `Requires Construction ${method.level}` : undefined}
    >
      <div className="flex flex-wrap items-start justify-between gap-2">
      ...
      <div className="flex flex-nowrap items-center gap-1 overflow-x-auto text-xs">
        ...
        {method.output ? (
          ...
          <span
            className="inline-flex shrink-0 items-center rounded-md border border-border/50 bg-secondary/20 px-1.5 py-1 text-[11px] text-muted-foreground"
            title="Built and removed in your POH — materials are consumed"
          >
            consumed
          </span>
        )}
