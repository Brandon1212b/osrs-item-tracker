...
import { PRAYER_METHODS, type PrayerMethod } from "@/lib/prayer-methods";
...
export function PrayerMethodsPanel({
  ...
  const ranked = useMemo(() => {
    const list: Ranked[] = PRAYER_METHODS.map((method) => {
      ...
      let locked = prayerLevel != null && prayerLevel < method.level;
    ...
    return list.sort((a, b) => {
      if (prayerLevel != null && a.locked !== b.locked) return a.locked ? 1 : -1;
      let cmp = 0;
      switch (sort) {
        case "gp_desc":
  ...
  }, [rowsByName, trendsById, g, sort, prayerLevel, magicLevel]);
...
function MethodRow({
  ...
  return (
    <article
      ...
      title={locked ? `Requires Prayer ${method.level}` : undefined}
    >
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div className="flex items-center gap-2.5">
          ...
          <div className="min-w-0">
            ...
            <p className={`text-[11px] ${locked ? "font-semibold text-amber-500/90" : "text-muted-foreground"}`}>
              {method.magicLevel != null ? `Magic ${method.magicLevel}` : `Prayer ${method.level}`}
              {locked ? " · locked" : ""}
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-nowrap items-center gap-1 overflow-x-auto text-xs">
        ...
        {method.output ? (
          ...
          <span
            ...
            title="Materials are consumed for Prayer XP"
          >
            consumed
          </span>
        )}
