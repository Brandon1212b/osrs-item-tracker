  ...
  const ranked = useMemo(() => {
    ...
    const list: Ranked[] = methods.map((method) => {
      ...
      for (const part of method.inputs) {
        ...
        if (p == null) {
          // One missing input price must not invalidate the whole method.
          missing = true;
          continue;
        }
      ...
      if (outParts.length > 0) {
        ...
        for (const part of outParts) {
          ...
          if (outUnit == null) {
            // Ignore unpriced drops; keep contributions from priced items.
            missing = true;
            continue;
          }
      ...
      return {
        ...
        netValuePerHour: null, // filled after refXp calibration
        missing, locked,
      };
    });
    ...
    // Continuous net-value scoring:
    // 1) refXpPerHour = best XP/h among methods the player can currently use
    // 2) xpValue = moneyPerHour / refXpPerHour  (implied GP value of 1 XP)
    // 3) netValuePerHour = gpPerHour + xpPerHour * xpValue
    ...
    return list.sort((a, b) => {
      // Unlocked methods always rank above locked ones so a locked method
      // cannot become the recommended (#1) entry when any unlocked exist.
      ...
      switch (sort) {
        ...
        case "cost_desc":
          // Higher "Your cost" first → lower net value first
          cmp = nullsLast(a.netValuePerHour, b.netValuePerHour, 1);
          break;
        case "cost_asc":
        default:
          // Recommended order: highest net value first (Pareto-consistent)
          cmp = nullsLast(a.netValuePerHour, b.netValuePerHour, -1);
          break;
      }
