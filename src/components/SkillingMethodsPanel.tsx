...
export type SkillingMethod = {
  ...
  /**
   * Expected outputs per action (fractional qty allowed).
   * When present, used instead of `output`. Coins are special-cased at 1 gp.
   */
  outputs?: MethodPart[];
};
...
export function SkillingMethodsPanel({
  ...
  const ranked = useMemo(() => {
    ...
    const list: Ranked[] = methods.map((method) => {
      ...
      // Multi-output support: prefer `outputs` array; fall back to single `output`.
      // Coins are always 1 gp (not a GE-traded item).
      ...
      let outValue: number | null = outParts.length === 0 ? 0 : 0;
      let baselineOutValue: number | null = outParts.length === 0 ? 0 : 0;

      if (outParts.length > 0) {
        let sum = 0;
        let baseSum = 0;
        let anyMissing = false;
        let anyBaseMissing = false;
        for (const part of outParts) {
          ...
          const doseScale =
            isHerblore && part.name.includes("(3)") ? expectedDoses / 3 : 1;
        ...
        <div className="flex flex-wrap items-start justify-between gap-2">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-muted-foreground">
              {description ??
                "Sorted by what each XP costs you — supplies plus the gold you could have made instead. Lower cost is better."}
              {skillLevel != null && (
                <>
                  {" "}
...
function PartChip({
  ...
  // Coins are always 1 gp — not a GE item.
  const unit =
    name === "Coins" ? 1 : kind === "input" ? buyPrice(row) : sellPrice(row);
